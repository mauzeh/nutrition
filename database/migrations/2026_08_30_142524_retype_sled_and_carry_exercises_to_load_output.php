...
return new class extends Migration
{
    ...
    {
        // 1. Update exercises and lift_logs scoped strictly by log_type IN ('sled', 'weighted-carry')
        ...
        // Also update lift_logs table if log_type is stored there
        if (Schema::hasColumn('lift_logs', 'log_type')) {
            DB::table('lift_logs')
                ->whereIn('log_type', ['sled', 'weighted-carry'])
                ->update(['log_type' => 'load_output']);
        }

        // 2. Recompute PRs for all users of affected exercises
    ...
    public function down(): void
    {
        ...
        if (Schema::hasColumn('lift_logs', 'log_type')) {
            DB::table('lift_logs')->where('exercise_type', 'sled')->update(['log_type' => 'sled']);
            DB::table('lift_logs')->where('exercise_type', 'static_hold')->update(['log_type' => 'weighted-carry']);
        }
    }
};
